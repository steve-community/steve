...
public class OcppTagRepositoryImpl implements OcppTagRepository {
    ...
    public List<Overview> getOverview(OcppTagQueryForm form) {
        ...
        selectQuery.addJoin(
                ...
                parentTable.ID_TAG.eq(OCPP_TAG_ACTIVITY.PARENT_ID_TAG));
    ...
    public Result<OcppTagActivityRecord> getRecords(List<String> idTagList) {
        return ctx.selectFrom(OCPP_TAG_ACTIVITY).where(OCPP_TAG_ACTIVITY.ID_TAG.in(idTagList)).fetch();
    }
    ...
    public void addOcppTagList(List<String> idTagList) {
        List<OcppTagRecord> batch =
                idTagList.stream()
                        .map(s -> ctx.newRecord(OCPP_TAG).setIdTag(s))
        ...
        try {
            ...
            if (e.getCause() instanceof SQLIntegrityConstraintViolationException) {
                throw new SteveException.AlreadyExists(
                        "A user with idTag '%s' already exists.", u.getIdTag());
            } else {
    ...
    private static class UserMapper
            implements RecordMapper<
                    Record10<
                            Integer, Integer, String, String, DateTime, Boolean, Boolean, Integer, Long, String>,
                    Overview> {
        @Override
        public Overview map(
                Record10<
