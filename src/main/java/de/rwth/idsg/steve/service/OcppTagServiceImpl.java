...
public class OcppTagServiceImpl implements OcppTagService {
    ...
    @Override
    public List<AuthorizationData> getAuthDataOfAllTags16() {
        int hoursToExpire = settingsRepository.getHoursToExpire();

        return ocppTagRepository.getRecords()
                .map(new AuthorizationDataMapper(hoursToExpire));
    }
    ...
    @Override
    public List<AuthorizationData> getAuthData16(List<String> idTagList) {
        int hoursToExpire = settingsRepository.getHoursToExpire();
...
//                idTagInfo.setStatus(ocpp.cs._2012._06.AuthorizationStatus.CONCURRENT_TX);
    ...
    @Override
    public ocpp.cs._2015._10.IdTagInfo getIdTagInfoV16(String idTag) {
        OcppTagRecord record = ocppTagRepository.getRecord(idTag);
        ocpp.cs._2015._10.IdTagInfo idTagInfo = new ocpp.cs._2015._10.IdTagInfo();

        if (record == null) {
            log.error("The user with idTag '{}' is INVALID (not present in DB).", idTag);
            idTagInfo.setStatus(ocpp.cs._2015._10.AuthorizationStatus.INVALID);
            processInvalid(idTag);
        } else {
            if (record.getBlocked()) {
                log.error("The user with idTag '{}' is BLOCKED.", idTag);
                idTagInfo.setStatus(ocpp.cs._2015._10.AuthorizationStatus.BLOCKED);
...
//                idTagInfo.setStatus(ocpp.cs._2012._06.AuthorizationStatus.CONCURRENT_TX);
                ...
                log.error("The user with idTag '{}' is EXPIRED.", idTag);
                idTagInfo.setStatus(ocpp.cs._2015._10.AuthorizationStatus.EXPIRED);

            } else {
                log.debug("The user with idTag '{}' is ACCEPTED.", idTag);
                idTagInfo.setStatus(ocpp.cs._2015._10.AuthorizationStatus.ACCEPTED);
    ...
    private static class AuthorizationDataMapper implements RecordMapper<OcppTagRecord, AuthorizationData> {
        private final DateTime nowDt;
        private final DateTime cacheExpiry;

        AuthorizationDataMapper(int hoursToExpire) {
            this.nowDt = DateTime.now();
            this.cacheExpiry = nowDt.plus(hoursToExpire);
        }
