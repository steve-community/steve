...
public final class ConnectorStatusFilter {
    ...
    public static List<ConnectorStatus> filterAndPreferOthersWithStatusOfZero(
            List<ConnectorStatus> initialList) {
        return processAndFilterList(initialList, Strategy.PreferOthersWithStatusOfZero);
    }

    /**
     * Problem description: For a station we have regular connectors ids > 0 and connector id 0
     * (represents whole station). Connector id 0 should have upper hand while providing status
     * information if its timestamp is more recent than the others and mask status info of other
     * connectors.
     *
     * <p>Link: https://github.com/steve-community/steve/issues/16
     *
     * <p>Logic walk-through:
     *
     * <p>For each station 0) group ConnectorStatus items around charge box ids, since input is a flat
     * list (mixed bag with all charge box ids and connectors) 1) find the latest ConnectorStatus of
     * connector id 0 (actually there can only be at most one) 2) find the latest ConnectorStatus
     * within the connectors ids > 0 3) compare the two, which timestamp is more recent: 0 or others?
     * 4) depending on the previous step, build the list to return
     */
    private static List<ConnectorStatus> processAndFilterList(
            List<ConnectorStatus> initialList, Strategy strategy) {
    ...
    private static List<ConnectorStatus> processForOneStation(
            List<ConnectorStatus> statsList, Strategy strategy) {
    ...
    private enum Strategy implements ZeroMoreRecentStrategy {
        PreferZero {
            @Override
            public List<ConnectorStatus> process(
                    List<ConnectorStatus> zero, List<ConnectorStatus> nonZero) {
                return zero;
            }
        },

        /**
         * If connector 0 is more recent, copy the status of connector 0 to other connector ids, and
         * return ONLY others.
         */
