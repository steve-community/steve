 ...
 *
 * <p>http://www.jooq.org/doc/3.4/manual/sql-execution/transaction-management/
 *
...
public class OcppServerRepositoryImpl implements OcppServerRepository {
    ...
    public void insertConnectorStatus(InsertConnectorStatusParams p) {
        ctx.transaction(
                configuration -> {
                    ...
                    log.debug(
                            "Stored a new connector status for {}/{}.", p.getChargeBoxId(), p.getConnectorId());
                });
    }
    ...
    @Override
    public void insertMeterValues(
            String chargeBoxIdentity, List<MeterValue> list, int transactionId) {
        if (CollectionUtils.isEmpty(list)) {
            return;
        }
        ...
        ctx.transaction(
                configuration -> {
                    try {
                        ...
                        int connectorPk =
                                ctx.select(TRANSACTION_START.CONNECTOR_PK)
    ...
    public int insertTransaction(InsertTransactionParams p) {
        ...
        if (unknownTagInserted) {
            log.warn(
                    "The transaction '{}' contains an unknown idTag '{}' which was inserted into DB "
                            + "to prevent information loss and has been blocked",
                    transactionId,
                    p.getIdTag());
        }
        ...
        if (p.isSetReservationId()) {
            reservationRepository.used(
                    connectorPkQuery, p.getIdTag(), p.getReservationId(), transactionId);
        }
    ...
    /**
     * Use case: If the station sends identical StartTransaction messages multiple times (e.g. due to
     * connection problems the response of StartTransaction could not be delivered and station tries
     * again later), we do not want to insert this into database multiple times.
     */
    private TransactionDataHolder insertIgnoreTransaction(
        ...
        try {
            Record1<Integer> r =
                    ctx.select(TRANSACTION_START.TRANSACTION_PK)
                            .from(TRANSACTION_START)
            ...
            Integer transactionId =
                    ctx.insertInto(TRANSACTION_START)
    ...
    /**
     * After a transaction start/stop event, a charging station _might_ send a connector status
     * notification, but it is not required. With this, we make sure that the status is updated
     * accordingly. Since we use the timestamp of the transaction data, we do not necessarily insert a
     * "most recent" status.
     *
     * <p>If the station sends a notification, we will have a more recent timestamp, and therefore the
     * status of the notification will be used as current. Or, if this transaction data was sent to us
     * for a failed push from the past and we have a "more recent" status, it will still be the
     * current status.
     */
    ...
    private void insertIgnoreConnector(DSLContext ctx, String chargeBoxIdentity, int connectorId) {
        int count =
                ctx.insertInto(CONNECTOR, CONNECTOR.CHARGE_BOX_ID, CONNECTOR.CONNECTOR_ID)
    ...
    /**
     * Use case: An offline charging station decides to allow an unknown idTag to start a transaction.
     * Later, when it is online, it sends a StartTransactionRequest with this idTag. If we do not
     * insert this idTag, the transaction details will not be inserted into DB and we will lose
     * valuable information.
     */
    private boolean insertIgnoreIdTag(DSLContext ctx, InsertTransactionParams p) {
        String note =
                "This unknown idTag was used in a transaction that started @ "
                        + p.getStartTimestamp()
                        + ". It was reported @ "
                        + DateTime.now()
                        + ".";
    ...
    private int getConnectorPkFromConnector(
            DSLContext ctx, String chargeBoxIdentity, int connectorId) {
    ...
    private void batchInsertMeterValues(
            DSLContext ctx, List<MeterValue> list, int connectorPk, Integer transactionId) {
        List<ConnectorMeterValueRecord> batch =
                list.stream()
                        .flatMap(
                                t ->
                                        t.getSampledValue().stream()
                                                .map(
                                                        k ->
                                                                ctx.newRecord(CONNECTOR_METER_VALUE)
                                                                        ...
                                                                        .setReadingContext(
                                                                                k.isSetContext() ? k.getContext().value() : null)
                                                                        .setFormat(k.isSetFormat() ? k.getFormat().value() : null)
                                                                        .setMeasurand(
                                                                                k.isSetMeasurand() ? k.getMeasurand().value() : null)
