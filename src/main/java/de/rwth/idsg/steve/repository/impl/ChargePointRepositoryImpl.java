...
public class ChargePointRepositoryImpl implements ChargePointRepository {
    ...
    public Optional<String> getRegistrationStatus(String chargeBoxId) {
        String status =
                ctx.select(CHARGE_BOX.REGISTRATION_STATUS)
                        .from(CHARGE_BOX)
    ...
    public List<ChargePointSelect> getChargePointSelect(
        ...
        Condition chargeBoxIdCondition =
    ...
    public ChargePoint.Details getDetails(int chargeBoxPk) {
        ChargeBoxRecord cbr =
                ctx.selectFrom(CHARGE_BOX).where(CHARGE_BOX.CHARGE_BOX_PK.equal(chargeBoxPk)).fetchOne();
    ...
    public List<ConnectorStatus> getChargePointConnectorStatus(ConnectorStatusForm form) {
        ...
        Table<?> t2 =
                ctx.selectDistinct(t2Pk, t2Ts, t2Status, t2Error)
                        .from(CONNECTOR_STATUS)
                        .join(t1)
        ...
        Condition chargeBoxCondition =
                CHARGE_BOX.REGISTRATION_STATUS.eq(RegistrationStatus.ACCEPTED.value());

        if (form != null && form.getChargeBoxId() != null) {
            chargeBoxCondition =
                    chargeBoxCondition.and(CHARGE_BOX.CHARGE_BOX_ID.eq(form.getChargeBoxId()));
        }
                ...
                .map(
                        r ->
                                ConnectorStatus.builder()
                                        ...
                                        .errorCode(r.value6())
                                        .ocppProtocol(
                                                r.value7() == null ? null : OcppProtocol.fromCompositeValue(r.value7()))
                                        .build());
    }
    ...
    public void addChargePointList(List<String> chargeBoxIdList) {
        List<ChargeBoxRecord> batch =
                chargeBoxIdList.stream()
                        .map(
                                s ->
                                        ctx.newRecord(CHARGE_BOX)
                                                .setChargeBoxId(s)
    ...
    private void deleteChargePointInternal(DSLContext ctx, int chargeBoxPk) {
        ctx.delete(CHARGE_BOX).where(CHARGE_BOX.CHARGE_BOX_PK.equal(chargeBoxPk)).execute();
    }
}
