...
import de.rwth.idsg.steve.web.dto.common.*;
...
public class Ocpp16Controller {
    ...
    @Autowired private ChargePointService16_Client client;
    ...
    private static final String SET_CHARGING_PRFL_PATH = "/SetChargingProfile";
    private static final String CLEAR_CHARGING_PRFL_PATH = "/ClearChargingProfile";
    private static final String GET_COMPOSITE_SCHDL_PATH = "/GetCompositeSchedule";
    ...
    private void setChargingRateUnit(Model model) {model.addAttribute("chargingRateUnit", ChargingRateUnitEnum.values());}

    private void setChargingProfilePurpose(Model model) {model.addAttribute("chargingProfilePurpose", ChargingProfilePurposeEnum.values());}

    private void setChargingProfileKind(Model model) {model.addAttribute("chargingProfileKind", ChargingProfileKindEnum.values());}

    private void setRecurrencyKind(Model model) {model.addAttribute("recurrencyKind", RecurrencyKindEnum.values());}
    ...
    public String getTriggerMessage(Model model) {
        ...
        return PREFIX + TRIGGER_MESSAGE_PATH;
    }
    ...
    @RequestMapping(value = SET_CHARGING_PRFL_PATH, method = RequestMethod.GET)
    public String getSetChargingPrfl(Model model) {
        setChargePointList(model);
        setChargingProfilePurpose(model);
        setChargingProfileKind(model);
        setRecurrencyKind(model);
        setChargingRateUnit(model);
        model.addAttribute(PARAMS, new SetChargingProfileParams());
        return PREFIX + SET_CHARGING_PRFL_PATH;
    }
    
    @RequestMapping(value = CLEAR_CHARGING_PRFL_PATH, method = RequestMethod.GET)
    public String getClearChargingPrfl(Model model) {
        setChargePointList(model);
        setChargingProfilePurpose(model);
        model.addAttribute(PARAMS, new ClearChargingProfileParams());
        return PREFIX + CLEAR_CHARGING_PRFL_PATH;
    }
    
    @RequestMapping(value = GET_COMPOSITE_SCHDL_PATH, method = RequestMethod.GET)
    public String getCompositeSchdl(Model model) {
        setChargePointList(model);
        setChargingRateUnit(model);
        model.addAttribute(PARAMS, new GetCompositeScheduleParams());
        return PREFIX + GET_COMPOSITE_SCHDL_PATH;
    }
    ...
    public String postTriggerMessage(@Valid @ModelAttribute(PARAMS) TriggerMessageParams params,
        ...
        return REDIRECT_TASKS_PATH + client.triggerMessage(params);
    }
    ...
    @RequestMapping(value = SET_CHARGING_PRFL_PATH, method = RequestMethod.POST)
    public String postSetChargingPrfl(@Valid @ModelAttribute(PARAMS) SetChargingProfileParams params,
                                   BindingResult result, Model model) {
        if (result.hasErrors()) {
            setChargePointList(model);
            setChargingProfilePurpose(model);
        ...
        return REDIRECT_TASKS_PATH + client.setChargingProfile(params);
    }
    
    @RequestMapping(value = CLEAR_CHARGING_PRFL_PATH, method = RequestMethod.POST)
    public String postClearChargingPrfl(@Valid @ModelAttribute(PARAMS) ClearChargingProfileParams params,
                                 BindingResult result, Model model) {
        if (result.hasErrors()) {
            setChargePointList(model);
            setChargingProfilePurpose(model);
        ...
        return REDIRECT_TASKS_PATH + client.clearChargingProfile(params);
    }
    
    @RequestMapping(value = GET_COMPOSITE_SCHDL_PATH, method = RequestMethod.POST)
    public String postCompositeSchdl(@Valid @ModelAttribute(PARAMS) GetCompositeScheduleParams params,
                                 BindingResult result, Model model) {
        if (result.hasErrors()) {
            setChargePointList(model);
            setChargingRateUnit(model);
        ...
        return REDIRECT_TASKS_PATH + client.getCompositeSchedule(params);
    }
}