...
CMD dockerize -wait tcp://mariadb:3306 -timeout 60s && \
    ./$(find . -type d -name "apache-maven-*" | head -n 1)/bin/mvn clean package -Pkubernetes -Djdk.tls.client.protocols="TLSv1,TLSv1.1,TLSv1.2" -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true -Dmaven.wagon.http.ssl.ignore.validity.dates=true \
